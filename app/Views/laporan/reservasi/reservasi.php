...
<div class="d-flex flex-wrap justify-content-between align-items-center mb-3 no-print">
    <h4 class="mb-0">Laporan Reservasi</h4>
...
<div class="card shadow-sm mb-3 no-print">
    <div class="card-body py-3">
        <div class="row g-2 align-items-end">
            <div class="col-auto">
                <select id="filterStatus" class="form-select form-select-sm" onchange="loadByStatus()">
...
function loadByStatus() {
    ...
    $.ajax({url: '<?= base_url('laporan-transaksi/reservasi/view') ?>', data: {status: status}, dataType: 'json', success: function(r) {
        dataLoaded = r.data || '';
...
function loadTanggal() {
    ...
    $('#tabelContent').html('<div class="text-center py-4"><div class="spinner-border"></div></div>');
    $.ajax({url: '<?= base_url('laporan-transaksi/reservasi/viewtanggal') ?>', type: 'POST', data: {tglmulai: a, tglakhir: b}, dataType: 'json', success: function(r) {
        dataLoaded = r.data || '';
        $('#tabelContent').html(dataLoaded || '<p class="text-muted text-center">Tidak ada data</p>');
    }});
}
function loadBulan() {
    ...
    $('#tabelContent').html('<div class="text-center py-4"><div class="spinner-border"></div></div>');
    $.ajax({url: '<?= base_url('laporan-transaksi/reservasi/viewbulan') ?>', type: 'POST', data: {bulan: bl, tahun: th}, dataType: 'json', success: function(r) {
        dataLoaded = r.data || '';
        $('#tabelContent').html(dataLoaded || '<p class="text-muted text-center">Tidak ada data</p>');
    }});
}
...
function cetakLaporan() {
    ...
    w.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Laporan Reservasi</title>' +
        ...
        '</style></head><body>' +
        '<div class="header"><h2>PENCUCIAN QENZA</h2><p>Sungai Jodi, Kec. Lubuk Tarok, Kabupaten Sijunjung</p><hr><div class="title">Laporan Reservasi</div></div>' +
        dataLoaded +
