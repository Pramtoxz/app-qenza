...
function loadByStatus() {
    ...
    $.ajax({url: '<?= base_url('laporan-transaksi/pencucian/view') ?>', data: {status: status}, dataType: 'json', success: function(r) {
        dataLoaded = r.data || '';
...
function loadTanggal() {
    ...
    $('#tabelContent').html('<div class="text-center py-4"><div class="spinner-border"></div></div>');
    $.ajax({url: '<?= base_url('laporan-transaksi/pencucian/viewtanggal') ?>', type: 'POST', data: {tglmulai: a, tglakhir: b}, dataType: 'json', success: function(r) {
        dataLoaded = r.data || '';
        $('#tabelContent').html(dataLoaded || '<p class="text-muted text-center">Tidak ada data</p>');
    }});
}
...
function cetakLaporan() {
    ...
    w.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Laporan Pencucian</title>' +
