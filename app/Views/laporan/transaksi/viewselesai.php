...
<?php else: ?>
    <?php
    $totalSemua = 0;
    foreach ($selesai as $s) { $totalSemua += ($s['totalbayar'] ?? 0); }
    ...
    if (($filter_type ?? '') === 'tahun'):
        ...
        foreach ($selesai as $s) {
            $bl = !empty($s['tglpencucian']) ? date('n', strtotime($s['tglpencucian'])) : 0;
            $blNama = $bulanNames[$bl] ?? '-';
            if (!isset($rekap[$blNama])) $rekap[$blNama] = 0;
            $rekap[$blNama] += ($s['totalbayar'] ?? 0);
        }
    ?>
    <table class="table table-bordered table-sm">
        ...
        <tbody>
            <?php $no = 1; foreach ($rekap as $blNama => $total): ?>
            <tr>
